...
async function seedCommunityData() {
    ...
    const mentors = await Promise.all([
        ...
        prisma.mentor.upsert({
            where: { email: 'vikram.shah@example.com' },
            create: {
                name: 'Vikram Shah',
        ...
        prisma.mentor.upsert({
            where: { email: 'meera.kapoor@example.com' },
            create: {
                name: 'Meera Kapoor',
    ...
    console.log(`✅ Created/Updated ${mentors.length} mentors`);
    ...
    const eventPromises = eventData.map(async (data) => {
        const existing = await prisma.communityEvent.findFirst({ where: { title: data.title } });
        if (!existing) {
    ...
    console.log(`✅ Created/Checked ${events.length} events`);
    ...
    const storyPromises = storyData.map(async (data) => {
        const existing = await prisma.successStory.findFirst({ where: { email: data.email } });
        if (!existing) {
    ...
    console.log(`✅ Created/Checked ${stories.length} success stories`);
    ...
    const resourcePromises = resourceData.map(async (data) => {
        const existing = await prisma.communityResource.findFirst({ where: { title: data.title } });
        if (!existing) {
            return prisma.communityResource.create({ data });
        }
        return existing;
    });
    const resources = await Promise.all(resourcePromises);
    console.log(`✅ Created/Checked ${resources.length} resources`);
